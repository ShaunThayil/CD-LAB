...
void init_minTable(int n_states){
...
void mark_unmarked(){
    ...
    for(int i=1;i<n_states;i++){
        for(int j=0;j<i;j++){
             printf("\nX->%d Y->%d\n",i,j);
            if(checkMark(i,j) == 1 && minTable[i][j] == 0){
                printf("\nMarked (%d,%d)",i,j);
                minTable[i][j] = 1;
                flag = 1;
...
void combine_next(int s1,int s2,int location,int next_pointer){
    for(int j=0;j<n_states;j++){
