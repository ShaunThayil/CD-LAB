     ...
     for(int i=0;i<n_states;i++){
         for(int j=1;j<n_inp_smb;j++){
           ...
           // printf("\nAdded Intermediate\n");
            ...
            index = 0;
            null_only = 1;
            for(int t=0;t<int_len;t++){
                   if(intermediate[t] != null){
                       null_only = 0;
                   }
            }
           // printf("\nNull Only :- %d\n",null_only);
            ...
            for(int l=0;l<int_len;l++){
                if(null == intermediate[l]){
                          continue;
                      }
                  //printf("Adding to NFA TRANSITIONS (%d,%d) -> %d",i,j)
                  for(int m=0;m<e_len[intermediate[l]];m++){
